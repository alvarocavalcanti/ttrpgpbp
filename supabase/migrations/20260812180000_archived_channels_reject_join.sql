-- UX#12: joining an archived channel must be rejected. Members can still read
-- it (ChannelView shows an archived/read-only banner), but no new joins.
...
REVOKE ALL ON FUNCTION join_channel(UUID, TEXT, TEXT, TEXT, TEXT, TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION join_channel(UUID, TEXT, TEXT, TEXT, TEXT, TEXT) TO authenticated;
