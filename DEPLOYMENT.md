...
- [ ] Any other static host works — point it at `dist/`, set the `VITE_*` vars, and make sure all routes fall back to `index.html` (SPA routing). Cloudflare Pages does this automatically. Whichever host you use, the app shell and the worker script must be served `Cache-Control: no-cache` (`/`, `/index.html`, `/sw.js` — `public/_headers` already does this on the reference host); a long-cached shell traps installed PWAs on the old version after an update, which the update banner cannot fix on its own.
