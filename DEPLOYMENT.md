...
| `VITE_CONTROLLER_NAME` | Optional data-controller name shown in the Privacy Policy and Terms footer (GDPR Art 13). Omit and the policies name the operator generically | Static host env (build) |
| `VITE_CONTROLLER_EMAIL` | Optional data-controller contact email shown next to the name. Omit and no contact email is shown | Static host env (build) |
