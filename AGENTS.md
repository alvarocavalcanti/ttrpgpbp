...
- **Zero-cost constraint — flag paid services up front**: the app generates no revenue and cannot use any paid service. Whenever research surfaces a tool, API, SDK, or hosted service with a price tag (including "free tier" hooks, usage-billed APIs, or keys that require a commercial agreement), say so explicitly in the same message that proposes it — name the cost model and who pays — and default to a free alternative or to not building it. Never register for, configure, or depend on a paid service without the User's explicit sign-off on the cost.
...
- After any UI change, check whether the help screenshots in `public/help/` need updating; regenerate and commit them when they do
  - Screenshot tooling lives in [`scripts/help-screenshots/`](scripts/help-screenshots/): `seed.sql` (idempotent local fixtures, password `shots-pass-1`) + `capture.mjs` (Playwright, 360×780 @3x light mode), which also writes the 320px WebP thumbnails in `public/help/thumbs/` used by the `/features` cards (regenerate standalone with `node scripts/help-screenshots/thumbs.mjs`). See its [README](scripts/help-screenshots/README.md) for the run steps. Review the regenerated PNGs and commit only the ones that changed.
