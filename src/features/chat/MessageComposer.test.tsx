...
describe('MessageComposer', () => {
  ...
  it('marks the message textarea as the mention combobox', () => {
    render(<MessageComposer isGM={false} members={members} onSendMessage={vi.fn()} />)

    const textarea = screen.getByRole('combobox', { name: 'Message' })
    // Closed: no popup attributes beyond the static expanded=false.
  ...
  it('points aria-controls at the rendered mention listbox id', () => {
    render(<MessageComposer isGM={false} members={members} onSendMessage={vi.fn()} />)
  ...
  it('moves aria-activedescendant to the highlighted option with ArrowDown', () => {
    const multiMembers: any[] = [
      { id: 'm1', user_id: 'u1', character_name: 'Hero', profile: { display_name: 'P1' } },
      { id: 'm2', user_id: 'u2', character_name: 'Archer', profile: { display_name: 'P2' } },
    ]
    ...
    // Stable ids derived from the listbox id and the option index.
    expect(hero.id).toMatch(/-option-0$/)
    expect(archer.id).toMatch(/-option-1$/)
    ...
    expect(hero).toHaveAttribute('aria-selected', 'false')
  })

  it('clears the combobox popup state when the mention list closes', () => {
    render(<MessageComposer isGM={false} members={members} onSendMessage={vi.fn()} />)
