...
export function AdminChannelView() {
  ...
  if (channelError) {
    return (
      <div className="p-8 text-center">
        ...
        <button type="button" onClick={() => refetchChannel()} className="font-semibold text-primary-600 dark:text-primary-400 hover:underline">Retry</button>
        <div className="mt-4"><Link to="/admin" className="text-sm text-primary-600 dark:text-primary-400 hover:underline">Back to admin</Link></div>
      </div>
    )
  }
  ...
  if (channelMissing || !channel) {
    return (
      <div className="p-8 text-center">
        <p className="text-surface-900 dark:text-surface-100 font-medium mb-2">Channel not found.</p>
        <p className="text-sm text-surface-500 mb-4">It may have been deleted or archived.</p>
        <Link to="/admin" className="text-sm text-primary-600 dark:text-primary-400 hover:underline">Back to admin</Link>
      </div>
    )
  }

  return (
    <div className="flex flex-col h-full w-full bg-white dark:bg-surface-800">
      <div className="p-4 border-b border-surface-200 dark:border-surface-700">
        <div className="flex items-center gap-3">
          <Link to="/admin" aria-label="Back to admin" className="p-2 -ml-2 text-surface-500 hover:bg-surface-100 dark:hover:bg-surface-700 rounded-full">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 19l-7-7 7-7" /></svg>
          </Link>
          <div className="flex-1 min-w-0">
            <h2 className="text-lg font-semibold text-surface-900 dark:text-surface-100 truncate">{channel.name}</h2>
            <div className="text-sm text-surface-500">{channel.game_system} · {channel.member_count} members</div>
          </div>
        </div>
        ...
        <details className="mt-3 text-sm">
          <summary className="cursor-pointer font-medium text-surface-700 dark:text-surface-300 hover:text-surface-900 dark:hover:text-surface-100 focus:outline-none focus:ring-2 focus:ring-primary-500 rounded">
            Players ({channel.member_count})
          </summary>
          <div className="mt-2">
            {membersLoading ? (
              <p className="text-surface-500">Loading players…</p>
            ) : membersError ? (
                ...
                {members.map(member => (
                  <li key={member.user_id} className="flex items-center gap-2 min-w-0">
                    ...
                    {member.display_name && member.display_name !== member.character_name && (
                      <span className="text-surface-500 truncate">{member.display_name}</span>
                    )}
                    {member.user_id === channel.gm_id && (
                      <span className="shrink-0 text-xs px-1.5 py-0.5 rounded bg-surface-100 dark:bg-surface-700 text-surface-600 dark:text-surface-300">GM</span>
                    )}
      ...
      <div ref={scrollRef} className="flex-1 w-full overflow-y-auto p-4 space-y-6">
        ...
        {hasMore && (
          <button type="button" onClick={() => void handleLoadOlder()} disabled={loadingOlder} className="w-full p-2 text-center text-sm text-primary-600 dark:text-primary-400 font-semibold hover:bg-surface-50 dark:hover:bg-surface-700 disabled:opacity-50">
        ...
        {loading && messages.length === 0 ? (
          ...
          <div className="text-center text-surface-500">
            <p className="font-medium text-surface-900 dark:text-surface-100 mb-1">No messages yet.</p>
            ...
            <div key={msg.id} className="flex gap-3">
              <div className="flex flex-col w-full min-w-0 max-w-[85%] items-start">
                <div className="flex items-baseline gap-2 mb-1 flex-wrap">
                  <span className="text-sm font-medium text-surface-900 dark:text-surface-100">{senderLabel(msg)}</span>
                  <span className="text-xs text-surface-500">{formatTimestamp(msg.created_at)}</span>
                  {msg.type !== 'regular' && (
                    <span className="text-xs px-1.5 py-0.5 rounded bg-surface-100 dark:bg-surface-700 text-surface-600 dark:text-surface-300">{msg.type}</span>
                  )}
                  {msg.npc_name && (
                ...
                <div className={`w-full px-4 py-2 rounded-2xl ${msg.is_deleted ? 'bg-surface-100 dark:bg-surface-700 text-surface-500 italic border border-surface-200 dark:border-surface-600' : 'bg-surface-100 dark:bg-surface-700 text-surface-900 dark:text-surface-100'}`}>
