...
- **Reliability and performance polish** — dice links only roll valid notation, the dice roller clamps its inputs as you type, double-tapping send can no longer post a message twice, message drafts survive browsers that block storage, a crash in one screen no longer takes the whole app down, the lobby's unread badges update live as messages arrive, and returning from the background no longer jumps your reading position. App downloads are also smaller since help screenshots are no longer pre-downloaded. After signing up, the lobby now waits for your sign-in to settle before loading your channels, so a brand-new account no longer risks a spurious "Failed to load channels" error.
