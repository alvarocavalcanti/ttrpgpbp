...
- **Your acceptance of the rules is now on record** — signing in records that you agreed to the Terms and Privacy Policy, and when those documents change you'll be asked to accept the new version once before continuing.
...
- **Mentioning a character lists names in alphabetical order** — typing `@` now shows characters A–Z (with `@all` still at the top for GMs), so finding the right person no longer means hunting through an unsorted list.
...
- **A tidier lobby** — each channel now previews its latest message with the sender's name in front, so you can tell at a glance who's talking. The row is cleaner too: a small **GM** tag sits next to the channel name when you run the game, and new messages show as a compact red count instead of the wordy "N new".
...
- **Search results now take you to the message** — clicking a result closes the search panel and scrolls the conversation to that message with a brief highlight, even when it sits far back in the channel's history. If the message can't be shown — because it was deleted, or the older history couldn't be loaded — a short notice says so instead of doing nothing.
...
- **Reliability and performance polish** — dice links only roll valid notation, the dice roller clamps its inputs as you type, double-tapping send can no longer post a message twice, message drafts survive browsers that block storage, a crash in one screen no longer takes the whole app down, the lobby's unread badges update live as messages arrive, and returning from the background no longer jumps your reading position. App downloads are also smaller since help screenshots are no longer pre-downloaded.
...
- **Report a message** — if another player posts something inappropriate, hover their message (or tap the **"⋯"** button on a phone) and choose **Report**. Describe the problem in your own words and send it to the server administrators, who'll see it together with the message and channel it came from. Works on any message posted by someone else; you'll get a clear confirmation when it goes through, or a chance to try again if it doesn't.
...
- **Server admins see who's who** — the Users tab in Server Admin now has a search box and a status filter (All / Active / Inactive / Suspended). Tap any user to open a detail view with their email (and whether it's verified), sign-in method, last login, last activity, message count, the channels they're in, and any past suspension history — and suspend or un-suspend them from the same place.
