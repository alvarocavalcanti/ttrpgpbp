...
- **Easier-to-tap message actions** — reply, edit, delete and X-Card are now one large "⋯" button on phones (opening a simple menu), instead of a row of tiny icons. Deleting a message asks for confirmation inside the app, so an accidental tap no longer removes it straight away.
