...
9. **P2 - Resolve mention ambiguity.** Mentions autocomplete by character-name prefix and linkification is case-sensitive. Duplicate character names can notify the wrong user or make identity unclear. Use stable member IDs in the composer representation and display disambiguating profile information. Evidence: `src/features/chat/MessageComposer.tsx:69-103`, `src/features/chat/mentions.ts:15-36`.
