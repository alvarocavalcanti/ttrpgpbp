...
- **Retry coverage** — Lobby, SearchModal (retryKey pattern), channel-level, messages banner, empty-list, ThreadList, ActiveGMs: every failed async surface now has an in-place Retry with `min-h-11` or clear labeling. This is the app's best-temperament error story to date.
