...
**Deduped totals: 0 P0 · 9 P1 · 28 P2 = 37 findings.** No cross-pillar merges (pillars did not overlap materially); one same-surface pairing noted (security P2-1 upload volume and release P2-4 rate limiting are the same free-tier abuse surface seen from two angles — folded into one backlog item).
...
| legal-2 | GA4 loads with no consent gate and was described as "anonymous". | Privacy wording corrected; release guidance to leave `VITE_GA_MEASUREMENT_ID` unset until a consent gate ships (see Go/No-go conditions and #604). |
...
| rel-4 | Documented backup RPO assumed Supabase Pro ($25/org/month) + PITR; the free tier has no backups. | `BACKUP_RESTORE.md` rewritten around the free-tier manual cadence + restore drill, with the Pro cost named; scheduled workflow deferred (#604). |

### P2 (28) — fixed here (9)

| # | Finding | Fix |
|---|---|---|
| sec-2 | Abuse reports had no de-duplication — a member could flood the admin System thread/pushes. | Partial unique index `(reporter_id, message_id)` (`20260925163924`); client maps `23505` to "You've already reported this message."; pgTAP. |
| sec-3 | GDPR export omitted `dice_roll_favorites` and the terms/age fields. | Added to `exportUserData` + tests. |
...
| legal-3 | Pre-age-gate accounts accepting via the re-consent gate never stamped `age_verified_at`. | `ReConsentGate` collects the 16+ attestation when needed; `ProtectedRoute` calls `confirmAge()`; tests. |
| legal-4 | Uploads ship disabled while the launch decision is enabled (undocumented toggle). | Documented as an explicit release step in `DEPLOYMENT.md`. |
| legal-5 | Historical changelog still claimed uploads are scanned. | Two entries annotated as removed 2026-09-24. |
| rel-5 | `DEPLOYMENT.md` omitted `VITE_SENTRY_DSN`; `BACKUP_RESTORE.md` referenced a non-existent `npm run test:e2e` and mixed dump APIs; `.env.example` shipped the maintainer's real controller identity. | Sentry row added; backup doc de-duplicated to `npx playwright test`; placeholders in `.env.example`. |
| — | (guard) The pre-commit typegen guard demanded `src/types/database.ts` for schema-neutral migrations. | Guard now exempts migrations whose added lines contain no schema-shape statement, with tests. |

### P2 (28) — deferred to [#604](https://github.com/alvarocavalcanti/ttrpgpbp/issues/604) (19)

Security P2-1 (unbounded upload volume) · Architecture P2-1–P2-6 (react-compiler direction [#567], PWA release-smoke/WebKit, shared CORS module, `safeStorage` adoption, dice-schema dedupe, render-time `Date.now`) · PbP P2-1–P2-4 (reconnect `client_request_id` collapse, server-stamped catch-up cursor, X-Card retry zero-clear, `retryMessage` no-id guard) · UX P2-1–P2-4 (PWA reload target, dice star, composer touch stragglers, re-consent focus trap) · Legal residual (GA consent gate) · Release P2-4 (rate limiting, with [#236](https://github.com/alvarocavalcanti/ttrpgpbp/issues/236)) and the scheduled backup workflow.
...
| GA consent (legal P1-2) vs release config (rel P1-3) | Distinct: copy is fixed; the consent gate / env decision is a release action + backlog item. |
...
**GO — with three operator actions at deploy.** No P0; every P1 is fixed and verified; the full suite (typecheck, lint, 1971 unit tests, coverage, build, markdownlint, cspell, 421 pgTAP tests, 7 E2E) is green.

Operator actions (not code, cannot be done from this branch):

1. **Analytics:** leave `VITE_GA_MEASUREMENT_ID` **unset** on the public instance until a prior-consent gate ships, or accept GA4 cookies/IP processing as-is. The Privacy copy no longer calls it "anonymous".
2. **Image uploads:** uploads ship **off**; flip `app_settings.image_uploading_enabled` in the admin console if the report-only launch posture is intended (documented in `DEPLOYMENT.md`). With uploads on, the unbounded-volume cap is still open (#604) — accepted risk on the free tier.
3. **Tag the release:** merge, then `git tag v1.0.0 && git push --tags` at the release commit.

Accepted residual risks: unscanned uploads with report-only moderation (disclosed in Terms/Privacy); no automated backups on the free tier (manual cadence documented); no rate limiting while the instance is invite-gated.

## GitHub issues

| Issue | Contents |
|---|---|
| [#604](https://github.com/alvarocavalcanti/ttrpgpbp/issues/604) | Post-1.0 hardening backlog — the 19 deferred P2s + 2 residual P1 items (GA consent gate, scheduled backup). |
...
4. Legal GA consent gate + release backup workflow — close the two residual P1s.
