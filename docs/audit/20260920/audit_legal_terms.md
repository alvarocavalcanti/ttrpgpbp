...
- `npx oxlint` → **passed** (exit 0; 46 react-compiler warnings, 0 errors)
...
**Problem:** Two distinct gaps. (a) The detection engine itself is unconfigured: `scan-upload/index.ts:90-96` refuses every upload (`status:"unavailable"`) until `SAFER_API_KEY` is set, and `logic.ts:36-41` states the Safer response shape "is not confirmed yet" — so today *nothing is scanned* and the feature is a fail-closed scaffold (safe, honest, but not operational). (b) When the operator *does* configure it, a real match — apparent CSAM — triggers the mandatory-reporting duty that US providers hold under 18 U.S.C. §2258A (and EU providers under the DSA/ePrivacy regime) with **zero automated or surfaced path to discharge it.** A silent audit row inside a per-user modal is not a report, and there is no alert to make the operator even look. The Terms are therefore more protective on paper than the system can deliver.
...
- **`verify_jwt = false` + shared-secret on `scan-upload`/`cleanup-images`/`push-notifications`** — carried from the security pillar; the DB trigger/service role is the intended caller.
