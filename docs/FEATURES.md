...
- **System thread (admin only)** — the server admin has a single **System** thread where safety alerts arrive: every new abuse report posts the reporter, reported user, channel, and reason with links, and every blocked upload posts the uploader, channel, path, and hash. The admin can reply in the thread to record the report filed with the authorities
