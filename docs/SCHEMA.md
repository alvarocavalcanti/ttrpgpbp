...
| `terms_accepted_at` | timestamptz, nullable | Terms acceptance timestamp, re-stamped by `confirm_terms(version)` on every explicit accept (re-consent gate only — never silently); browser roles cannot write it (trigger guard) |
